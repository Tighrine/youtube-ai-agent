...
const ChatRow = ({
  ...
  const { closeMobileNav, openMobileNav } = use(NavigationContext);
  ...
  return (
    <div
      className="group rounded-xl border border-gray-200/30 bg-white/50 backdrop-blur-sm hover:bg-white/80 transition-all duration-200 
      ...
      <div className="p-4">
        ...
        {lastMessage && (
          <p className="text-xs text-gray-400 mt-1.5 font-medium">
            <TimeAgo timestamp={new Date(lastMessage.createdAt)} />
          </p>
        )}
      </div>
    </div>
